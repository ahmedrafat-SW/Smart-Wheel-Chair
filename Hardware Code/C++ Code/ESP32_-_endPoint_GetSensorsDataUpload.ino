...
OneWire  ds(2); 
void initNet();
int sendSensorsData(String httpRequestData ,String serverName); 
int sendData(String httpRequestData , String serverName);                                                
void getLoc();
void getTempC_F();
int getDistance();
...
void loop() {
  //  Checking wireless network connection
    ...
    if (getDistance()> 10 || getDistance()== 0 || celsius > 32 && gps.time.isValid()){
        ...
        ResponseCode = sendData(reqData , serverName);
        if (ResponseCode == 200){
